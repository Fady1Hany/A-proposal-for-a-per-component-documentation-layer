...
So, literally, what needs to be present in the AI's context window is agent.md ( which contains many instructions one of them is the loop which is described in [Readme](https://github.com/Fady1Hany/A-proposal-for-a-per-component-documentation-layer/blob/main/README.md) ) , component_NAME.md , the code of the component it is currently modifying, and the saved path or copy of that component's code from before the modification.
